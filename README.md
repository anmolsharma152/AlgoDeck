...
- `TRUST_PROXY`: Configures Express proxy IP trust evaluation (default: `loopback` / `false` for bare local runs; set to explicit proxy IP or exact bridge subnet e.g. `loopback,172.18.0.0/16` when deployed behind reverse proxies like Caddy).
